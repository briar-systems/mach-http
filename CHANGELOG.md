...
### Changed
- **Breaking.** Dependencies: mach-std 5.0.0, pinned to its `next/5` integration branch until the release is tagged. It requires mach 5.2 or later. The h3-quic tests override std the same way until mach-quic 0.12.0, which targets std 5, is pinned.
- **Breaking.** Every caller-supplied `now` and every deadline is a `std.chrono.time.Instant`, read with `time.instant()`, instead of a `time.Time`. This covers `h1.connection` (`init`, `tick`, `process`, `complete_io`, `enqueue_request`, `prepare_informational`, `prepare_response`, `release`, `Deadline.at`, `next_deadline`), the client (`next`, maintenance, `expire_request`, retry and readiness instants and their timer actions), the DNS cache (lookups, TTL and retry instants, resolver scope deadlines) and the connection pool (`take_close`, idle deadlines, last-use ordering). A wall-clock value no longer compiles where a deadline is expected, which replaces the 0.11.0 note that such a value was accepted but never fired.
